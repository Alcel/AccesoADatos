...
public class Empresa {
	...
	public static void main(String[] args) {
		...
		//modificador(7,"Rudolfo",200.3);
		//eliminador(2);
		lector();

	}

	public static void eliminador(int id) { //Problema en sustituir por 0, peor no por otros numeros
	...
	public static void intercambiador(int idEmp1, int idEmp2) { /* Esta a medio hacer */
		 ...
		 * while(raf.getFilePointer()<raf.length()) { idEmp1 = raf.readInt(); for(int
		 * i=0; i<NOMBRE_LENGTH;i++) { nombre[i]=raf.readChar(); } sNombre =
		 * String.valueOf(nombre).trim(); edad = raf.readInt(); for(int i = 0;
		 * i<cargo.length;i++) { cargo[i]=raf.readChar(); } sCargo =
		 * String.valueOf(cargo).trim(); sueldo = raf.readDouble();
		 * System.out.printf("\n%11d%10s%5d%15s  %5.2f",numEmpleado, sNombre, edad,
		 * sCargo, sueldo); } raf.close();
		 * 
		 * 
